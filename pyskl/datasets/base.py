...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate2(self,
            ...
            for ann in self.video_infos:
                ...
                frame_dirs.append(ann['frame_dir'])
            
            max_k_preds = np.argsort(results, axis=1)[:, -1:][:, ::-1]
            max_k_preds=max_k_preds.T
            #创建csv保存结果
            output_filename = os.path.join(work_dir, 'Submission.csv')
            output_file = open(output_filename, 'w')
            for i in range(len(frame_dirs)):
                sub=frame_dirs[i]+','+str(max_k_preds[0][i])+'\n'
                output_file.write(sub)
            output_file.close()
            zip_file_name = os.path.join(work_dir,'Submission.zip')
        ...
        # Create a ZipFile object with the output zip file name and mode
            ...
            # frame_dir_and_labels=[[ann['frame_dir'],ann['label']] for ann in self.video_infos]
            # gt_labels = [ann['label'] for ann in self.video_infos]
            # frame_dirs=[ann['frame_dir'] for ann in self.video_infos]
