...
class RecognizerGCN(BaseRecognizer):
    ...
    def forward_train(self, keypoint, emb,label, **kwargs):
        ...
        cls_score,emb_score = self.cls_head(x)
        gt_label = label.squeeze(-1)
        loss = self.cls_head.loss(cls_score, gt_label,emb_score,emb)
        losses.update(loss)
    ...
    def forward(self, keypoint, emb=None,label=None, return_loss=True, **kwargs):
        """Define the computation performed at every call."""
        if return_loss:
            if label is None:
                raise ValueError('Label should not be None.')
            return self.forward_train(keypoint, emb,label, **kwargs)
